...
df_pop <- df_pop %>% left_join(df_urban_rural %>% select(datazone2011,ur2_2016))
...
df_pop_0_17_simd_1 <- df_pop %>% 
  filter(hscp2019name==hscp_of_interest,
         ...
         simd2020v2_sc_quintile==1) %>% # Scotland quintiles?
  group_by(hscp_locality) %>% 
  summarise(pop_0_17_simd_1=sum(pop))



df_pop_0_17_urban <- df_pop %>% filter(hscp2019name==hscp_of_interest,
             age %in% ages_of_interest) %>% 
...
df_pop_0_17_summary <- df_pop_0_17_summary %>% 
  mutate(pop_0_17_simd_1_prop=round(pop_0_17_simd_1/pop_0_17,2),
...
cs_cols <- CellStyle(wb) + Font(wb, isBold = TRUE) + Border() # header

sheet_pop_0_17_summary <- createSheet(wb, sheetName = "pop_0_17_summary")
